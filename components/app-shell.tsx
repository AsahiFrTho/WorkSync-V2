...
export function AppShell({ children }: { children: React.ReactNode }) { const pathname = usePathname(); const { session, role, loading } = useSessionContext(); const roleInfo = ROLES[role]; const RoleIcon = ROLE_ICONS[role]; const navigation = NAVIGATION[role]; const [moreOpen, setMoreOpen] = useState(false); const name = loading && !session ? 'Verifying session…' : session?.name || roleInfo.label; return <div className="flex min-h-screen w-full bg-background font-sans print:block"><a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-50 focus:rounded-control focus:bg-primary focus:px-4 focus:py-3 focus:text-primary-foreground">Skip to main content</a>
  ...
  <div className="flex min-w-0 flex-1 flex-col"><header className="sticky top-0 z-20 flex min-h-14 items-center justify-between gap-4 border-b border-border bg-card px-4 sm:px-6 lg:px-8 print:hidden"><div className="lg:hidden"><Brand /></div><div className="hidden items-center gap-2 text-xs text-muted-foreground lg:flex"><ShieldCheck className="size-4 text-primary" /> {roleInfo.label} <span className="text-border">/</span> <span className="text-foreground">{pathname === '/dashboard' || pathname === '/dashboard-v2' ? 'State command center' : pathname === '/analytics' ? 'Statewide analytics' : pathname === '/skillgaps' ? 'Skill gap intelligence' : pathname === '/trainee' ? 'Trainee outcome passport' : pathname === '/insights' ? 'AI policy insights' : pathname === '/learners' ? 'Learner registry' : pathname.slice(1) || 'Overview'}</span></div><GlobalSearch className="ml-auto hidden w-full max-w-sm md:flex" /><div className="flex items-center gap-2"><span className="hidden rounded-control border border-border px-2 py-1 text-[10px] text-muted-foreground sm:inline-flex">{roleInfo.shortLabel}</span><AccessibilityControls /><ThemeToggle /><SignOutButton className="hidden rounded-control border border-border px-3 py-1.5 text-xs text-muted-foreground hover:bg-muted hover:text-foreground sm:inline-flex" /></div></header><div className="flex gap-1 overflow-x-auto border-b border-border bg-card px-4 py-2 lg:hidden print:hidden">{navigation.primary.map((item) => <Link key={item.href} href={item.href} className={cn('whitespace-nowrap rounded-control px-3 py-1.5 text-xs', isActive(pathname, item.href) ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted')}>{item.label}</Link>)}</div><main id="main-content" className="min-w-0 flex-1">{children}</main></div></div> }
