...
export function DataTable<T extends { id: string }>({ columns, rows, onRowClick, onSort }: { columns: Column<T>[]; rows: T[]; onRowClick?: (row: T) => void; onSort?: (key: keyof T) => void }) { return <div className="overflow-x-auto rounded-card border border-border"><table className="w-full border-collapse text-left text-body"><thead className="bg-muted text-meta uppercase tracking-[0.08em] text-muted-foreground"><tr>{columns.map((column) => <th key={String(column.key)} scope="col" className={cn('px-4 py-3 font-semibold', column.numeric && 'tabular-nums text-right')}><span className={cn('inline-flex items-center gap-2', column.numeric && 'justify-end')}>{column.sortable && onSort ? <button type="button" onClick={() => onSort(column.key)} className="inline-flex items-center gap-2 hover:text-foreground focus-visible:outline-2 focus-visible:outline-brand">{column.header}<ArrowUp aria-hidden="true" /></button> : column.header}</span></th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.id} tabIndex={onRowClick ? 0 : undefined} onClick={() => onRowClick?.(row)} onKeyDown={(event) => { if (onRowClick && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); onRowClick(row) } }} className={cn('border-t border-border', onRowClick && 'cursor-pointer transition-ui hover:bg-canvas focus-visible:bg-canvas focus-visible:outline-2 focus-visible:outline-brand')}>{columns.map((column) => <td key={String(column.key)} className={cn('px-4 py-3 text-foreground', column.numeric && 'text-right tabular-nums')}>{column.render ? column.render(row[column.key], row) : String(row[column.key])}</td>)}</tr>)}</tbody></table></div> }
...
export function ChartFrame({ title, description, unit, source, children }: { title: string; description?: string; unit?: string; source?: string; children: React.ReactNode }) { return <SectionCard title={title} description={description}><div className="min-h-48">{children}</div>{unit || source ? <div className="flex flex-wrap justify-between gap-2 border-t border-border px-5 py-3 text-meta text-muted-foreground">{unit ? <span>Unit: {unit}</span> : <span />}{source ? <span>Source: {source}</span> : null}</div> : null}</SectionCard> }
