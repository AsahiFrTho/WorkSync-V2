...
export function StatTile({ label, value, metadata, icon: Icon = PanelRight, trend, polarity = 'higher-is-better', onClick }: { label: string; value: string | number; metadata?: string; icon?: LucideIcon; trend?: { value: string; direction: 'up' | 'down' | 'flat' }; polarity?: 'higher-is-better' | 'lower-is-better' | 'neutral'; onClick?: () => void }) {
  ...
  const trendNode = trend ? <span className={cn('mt-4 inline-flex items-center gap-1 rounded-pill border px-2 py-1 text-meta font-medium', trendClass)}><span aria-hidden="true">{trendIcon}</span>{trend.value}</span> : null
